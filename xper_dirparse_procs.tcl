...
#           20170328 TLZ - v2 - (désactivé - à tester) Ajout du mode Multi permettant de récupérer plusieurs pièces jointes (-all)
#           20170811 TLZ - v2 - Recherche sans tenir compte des majuscules pour le mode simple (?i)
#           Amélioration nécessaires : échapper les caractères spéciaux des noms des fichiers
...
######################################################################
# Name:      xper_dirparse_alerte_nb_file
# Purpose:   Procs permettant de compter les fichiers en retour d'une dirparse. Si nombre de message ne correspond $mh est vidé, veillez à placer la proc en dirpase TPS Inbound après une dirparse
# UPoC type: tps
# Args:      tps keyedlist containing the following keys:
#            MODE    run mode ("start", "run", "time" or "shutdown")
#            MSGID   message handle
...
#               2019 04 19 IHB please call this proc from inbound dirparse TPS
# Exemple: {DEBUG 1} {SERVER serversmtp.com} {PORT 25} {USERNAME login]} {PASSWORD password} {USETLS 0} {FROM cloverleaf@infor.com} {TO support@infor.com,infor@infor.com} {SUBJECT sujet} {BODY Corp du mail} {NBFILE 13}
#    {DEBUG 0} {SERVER serveursmtp.fr} {PORT 25} {FROM cloverleaf@site.fr} {TO cloverleaf@infor.com,support@xperis.fr} {SUBJECT "Cloverleaf: nombre de fichier" } {BODY "Les fichiers ne sont pas prélevés car le nombre de fichier en attente de prélèvement ne correspond pas. Veuillez vérifier Merci." } {NBFILE 13}
######################################################################
# Name:      xper_dirparse_alerte_nb_file
# Purpose:   <description>
# UPoC type: tps
...
proc xper_dirParseUsingRegexp_v62_delay { args } {
    ...
    switch -exact -- $mode {
        ...
        run {
            ...
            if {$debug} {echo patternListList avant délai : $patternListList}
                    ...
                    if { $protocol == "fileset/local-tps" } {
                        ...
                        if { [cequal [file exists $myfile] 1] } {
                            ...
                            if { $rc == 0 } {
                                ...
                                if { [expr [clock seconds] - $scan_date] >= $delay } {
                                    lappend delayList $patternListElement
                                    if {$debug} {echo delay respecté pour fichier : $myfile}
                                    if {$debug} {echo delay respecté pour la liste en cours : $delayList}
                                } else {
                                    if {$debug} {echo delay non respecté pour le fichier : $myfile}
                                }
                            }
                        } else {
            ...
            if {$debug} { echo delay respecté pour la liste definitive suivante,delayList : $delayList}
            # End of Delay
